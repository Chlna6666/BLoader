...
const PREMAIN_FAILURE_EXIT_CODE: u32 = 0xE026_0001;
...
pub unsafe extern "system" fn DllMain(
    ...
    install_minimal_crash_handlers();
...
unsafe fn install_minimal_crash_handlers() {
    let _ = AddVectoredExceptionHandler(1, Some(diagnostic_crash_veh));
    let _ = SetUnhandledExceptionFilter(Some(diagnostic_unhandled_filter));
    let _ = _set_invalid_parameter_handler(Some(diagnostic_invalid_parameter));
    let _ = _set_purecall_handler(Some(diagnostic_purecall));
}

unsafe extern "system" fn diagnostic_crash_veh(_exception: *mut EXCEPTION_POINTERS) -> i32 {
    EXCEPTION_CONTINUE_SEARCH
}

unsafe extern "system" fn diagnostic_unhandled_filter(_exception: *const EXCEPTION_POINTERS) -> i32 {
    EXCEPTION_EXECUTE_HANDLER
}

unsafe extern "C" fn diagnostic_invalid_parameter(
...
unsafe extern "C" fn diagnostic_purecall() {}
...
unsafe extern "system" fn oep_gate_veh(exception: *mut EXCEPTION_POINTERS) -> i32 {
    ...
    if entry_point == 0
        || record.ExceptionAddress as usize != entry_point
        || get_current_thread_id() != STARTUP_THREAD_ID.load(Ordering::Acquire)
    {
        return EXCEPTION_CONTINUE_SEARCH;
    }
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        (*source_context).Rip = gate_dispatch as *const () as usize as u64;
    }
...
unsafe extern "system" fn gate_dispatch() -> ! {
    if !CONTEXT_SAVED.load(Ordering::Acquire)
        || get_current_thread_id() != STARTUP_THREAD_ID.load(Ordering::Acquire)
    {
        terminate_for_gate_failure();
    }
...
unsafe fn pe_entry_point(image_base: usize) -> Option<usize> {
    let dos_magic = ptr::read_unaligned(image_base as *const u16);
    if dos_magic != IMAGE_DOS_SIGNATURE {
        return None;
    }

    let lfanew = ptr::read_unaligned((image_base + 0x3C) as *const i32);
    if lfanew <= 0 {
        return None;
    }
    let nt = image_base.checked_add(lfanew as usize)?;
    ...
    let rva = ptr::read_unaligned((optional + 0x10) as *const u32) as usize;
    if rva == 0 {
        return None;
    }
